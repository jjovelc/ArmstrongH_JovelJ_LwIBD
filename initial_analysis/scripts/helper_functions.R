...
### CircosPlot and its helper functions from mixomics package is modified to be used in unsupervised methods like sgcca. 
###In the package it only works with Daiblo method

circosPlot_sgcca<-function (object, comp = 1:min(object$ncomp), cutoff, color.Y, 
          color.blocks, color.cor, var.names = NULL, showIntraLinks = FALSE, 
          line = FALSE, size.legend = 0.8, ncol.legend = 1, size.variables = 0.25, 
          size.labels = 1, legend = FALSE) 
{
  ...
  Y = object$Y
  indY = object$indY
  #object$variates =object$variates
  ...
  corMat = do.call(rbind, lapply(simMatList, function(i) do.call(cbind, 
                                                                 i)))
  Xdat = as.data.frame(do.call(cbind, X)[, colnames(corMat)])
  AvgFeatExp0 = Xdat %>% gather(Features, 
                                                  Exp) %>% group_by(Features) %>% dplyr::summarise(Mean = mean(Exp), 
                                                                                                          SD = sd(Exp))
  AvgFeatExp0$Dataset = factor(rep(names(X), unlist(lapply(cord, 
                                                           nrow))), levels = names(X))[match(AvgFeatExp0$Features, 
                                                                                             colnames(Xdat))]
  featExp = AvgFeatExp0 %>% group_by(Dataset) %>% arrange(Mean)
                     ...
                     size.legend=1.5)
{
  ...
  # check input object
  if (!is(object, "sgcca"))
    stop("cim_sgcca is only available for 'sgcca' objects")
  ...
  #indY = object$indY
  #object$variates = c(object$variates, object$variates)
  #object$loadings = c(object$loadings, object$loadings)
  ...
  XDatList = mapply(function(x, y){
    x[, y]
  }, x=X, y=keepA, SIMPLIFY=FALSE)
  XDat = do.call(cbind, XDatList)
  XDat[which(XDat > 2)] = 2
  XDat[which(XDat < -2)] = -2
  
  #dark = brewer.pal(n = 12, name = 'Paired')[seq(2, 12, by = 2)]
  #print(rep(names(X),lapply(keepA, sum)))
  VarLabels = factor(rep(names(X), lapply(keepA, sum)),
  ...
  cim(XDat,transpose= transpose, color = color,
      ...
      col.sideColors = color.blocks[as.numeric(VarLabels)],margins = margins)
